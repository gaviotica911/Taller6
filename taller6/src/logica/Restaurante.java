...
public class Restaurante
{
	...
	public void iniciarPedido(String nombreCliente,String direccionCliente,int idPedido) throws PrecioProductoException
	{	
		...
		while(pedidosEnCurso==1) {
			...
			switch (dato) 
			{
				case 1: int combo=Integer.parseInt(input("Ingrese el numero del combo que desea ordenar: "));
						...
						catch(PrecioProductoException e)
						{
			...
			pedidosEnCurso=Integer.parseInt(input("�Desea ordenar algo m�s? SI(1) NO(0). Si elige no, se cerrar� el pedido: "));
		}
	...
	public void cargarInformacionRestaurante(File archivoBebidas, File archivoIngredientes, File archivoMenu, File archivoCombos)throws IOException, HamburguesaException
	{
	...
	private void cargarMenu(File archivoMenu)throws IOException, ProductoRepetidoException
	{
		...
		while (linea != null) // Cuando se llegue al final del archivo, linea tendr� el valor null
		{
			...
			if (elProducto != null) 
			{
				ProductoRepetidoException ex = new ProductoRepetidoException(nombreProducto);
				throw ex;
			}
			else
			{
